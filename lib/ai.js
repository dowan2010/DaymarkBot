...
async function geminiGenerate(modelName, prompt, timeoutMs) {
  ...
  // fetch를 AbortController로 실제 취소함 — 예전엔 Promise.race로 타임아웃만 먼저
  // 반환하고 원래 fetch는 백그라운드에서 계속 돌다가, 한참 뒤(undici가 여러 IP를
  // 순차 시도하다) 실패하면 주인 없는 요청이 unhandledRejection으로 튀어나와
  // "fetch failed"로 화면이 깨지는 원인이었음
  ...
  try {
    ...
    if (err.name === 'AbortError') throw new Error('AI 응답 시간 초과');
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
export async function generateWithRetry(prompt, timeoutMs, retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      return await geminiGenerate(MODEL, prompt, timeoutMs);
    } catch (err) {
      // 재시도해도 소용없는 것만 예외로 두고(잘못된 요청/키), 나머지(fetch 자체 실패 포함
      // status 없는 네트워크 에러, 429, 500/503, 타임아웃, 과부하 등)는 전부 재시도
      const isRetryable = err.status !== 400 && err.status !== 401 && err.status !== 403;
